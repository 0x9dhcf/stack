...
SetClientActive(Client *c, Bool b)
{
    ...
    if (b && !c->isActive) {
        ...
        if (!(c->types & NetWMTypeFixed))
            for (int i = 0; i < 4; ++i)
                XGrabButton(display, Button1, Mod | modifiers[i], c->window,
...
MaximizeClient(Client *c)
{
    if (!(c->types & NetWMTypeFixed) && !IsFixed(c->normals) && !c->isTiled) {
        ...
        MoveResizeClientFrame(c, d->wx, d->wy, d->ww, d->wh, False);
        SetNetWMStates(c->window, c->states);
    }
}
...
FullscreenClient(Client *c)
{
    if (!(c->types & NetWMTypeFixed) && !IsFixed(c->normals) && !c->isTiled) {
        ...
        MoveResizeClientWindow(c, m->x, m->y, m->w, m->h, False);
        SetNetWMStates(c->window, c->states);
        RaiseClient(c);
    }
}
...
MoveClientToDesktop(Client *c, int desktop)
{
    ...
    c->isTopbarVisible = d->showTopbars;
    c->desktop = desktop;
...
{
    MoveClientToMonitor(c, NextMonitor(c->monitor));
}

void
MoveClientToPreviousMonitor(Client *c)
{
    MoveClientToMonitor(c, PreviousMonitor(c->monitor));
}
